...
def pdftoimg(comp_path):
        try:
            project_name = comp_path[63:]
            print(project_name)
            
            images = convert_from_path(comp_path)
            proj_counter = 0
            uid = createproject(project_name)           
...
def start_con(parsed_data):

    Semaphr = 1

    print('\n')
    path_addon = parsed_data[0]['data']["$undefined$"]
    
    # for linux -ubuntu path

    # /home/divyam/.local/share/label-studio/media/upload/

    base_path = '/home/divyam/.local/share/label-studio/media'
    comp_path = base_path+path_addon[5:]
    print('\n')
    pdftoimg(comp_path)
    Semaphr = 0
